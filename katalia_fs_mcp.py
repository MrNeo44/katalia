...
# Py >= 3.10 has Match; keep compatibility for older runtimes
...
def _ruff_unused_symbols(repo: Path, rel_path: str) -> Optional[int]:
    ...
    FIX: use JSON output (stable) instead of --quiet parsing.
    """
    try:
        res = subprocess.run(
            [
        ...
        if not out:
            # Could be a ruff/runtime error; don't lie
            return None
...
def _analyze_entities(code: str, use_radon: bool) -> Dict[str, Any]:
    ...
    _ = (magic_count, max_ident)  # silence lint; details are returned via file_examples
...
class _AstStatsVisitor(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> Any:
        # FIX: count class statement + traverse body
        self.stmt_count += 1
        self.n_classes += 1
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> Any:
        ...
        # FIX: do not stop traversal; count statements inside
...
def _compute_metrics_for_code(code: str, require_radon: bool, use_radon: bool) -> Dict[str, Any]:
    ...
    if (not use_radon) or (not RADON_AVAILABLE):
        ...
        sloc = loc - blank  # approx

        # FIX: give meaningful cc_blocks/cc_avg without radon
        blocks = int(astm.get("n_functions", 0)) + int(astm.get("n_methods", 0))
        cc_sum = float(astm.get("ast_cyc_sum", 0.0) or 0.0)
        cc_max = float(astm.get("ast_cyc_max", 0.0) or 0.0)
        cc_avg = float(cc_sum / blocks) if blocks > 0 else 0.0
...
def analyze_python_repo_full(
    ...
    return {
        ...
        "artifact": artifact,
        "files": len(files),
        "repo_metrics_keys": sorted(list(repo_out.keys())),
        "batch": batch,
    }
